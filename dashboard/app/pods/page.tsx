...
export default function PodsPage() {
  ...
  const load = async (soft = false) => {
    ...
    try { setPods(await getPods()) } finally {
      setLoading(false)
      setRefreshing(false)
    }
  }
  ...
  const handleCreate = async () => {
    if (!name.trim() || !image.trim()) { setError('Name and image are required.'); return }
    setCreating(true); setError('')
    try {
      await createPod(name.trim(), image.trim())
      setModalOpen(false); setName(''); setImage('')
      load(true)
  ...
  const handleDelete = async () => {
    if (!confirmId) return
    setDeleting(true)
    try { await deletePod(confirmId); load(true) }
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-text-primary text-3xl font-semibold tracking-tight mb-1">Pods</h1>
          <p className="text-text-muted text-sm">{pods.length} pod{pods.length !== 1 ? 's' : ''} in cluster</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border-subtle rounded-xl overflow-hidden">
          ...
          <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-border-subtle">
            ...
            <span className="col-span-3 text-text-muted text-xs font-medium uppercase tracking-wider">Image</span>
            <span className="col-span-2 text-text-muted text-xs font-medium uppercase tracking-wider">Status</span>
            <span className="col-span-3 text-text-muted text-xs font-medium uppercase tracking-wider">Node</span>
            <span className="col-span-1" />
          </div>
          {/* Rows */}
          <ul className="divide-y divide-border-subtle">
            {pods.map(pod => (
              <li key={pod.id} className="grid grid-cols-12 gap-4 px-5 py-3.5 items-center hover:bg-overlay/40 transition-colors group">
                <div className="col-span-3 flex items-center gap-2.5 min-w-0">
                  <div className="w-6 h-6 rounded-md bg-surface border border-border-subtle flex items-center justify-center shrink-0">
                    <Box size={11} strokeWidth={1.5} className="text-text-muted" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-text-primary text-sm font-medium truncate">{pod.name}</p>
                    <p className="text-text-muted text-xs font-mono truncate">{pod.id.slice(0, 8)}…</p>
                  </div>
                </div>
                <p className="col-span-3 text-text-secondary text-sm font-mono truncate">{pod.image}</p>
                <div className="col-span-2">
                  <StatusBadge status={pod.status} />
                </div>
                <p className="col-span-3 text-text-secondary text-sm font-mono truncate">{pod.node_id || '—'}</p>
      ...
      <Modal
        open={modalOpen}
        onClose={() => { setModalOpen(false); setError(''); setName(''); setImage('') }}
        title="Create pod"
        description="Deploy a new pod to the cluster."
      >
        <div className="space-y-4">
          <div>
            <label className="text-text-secondary text-xs font-medium block mb-1.5">Pod name</label>
            <input
              value={name}
          ...
          <div>
            <label className="text-text-secondary text-xs font-medium block mb-1.5">Image</label>
            <input
              value={image}
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              onClick={() => { setModalOpen(false); setError(''); setName(''); setImage('') }}
            ...
            >
              {creating && <RefreshCw size={13} strokeWidth={1.5} className="animate-spin" />}
              Create
            </button>
          </div>
        </div>
