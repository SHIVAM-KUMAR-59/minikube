...
export default function ServicesPage() {
  ...
  const load = async (soft = false) => {
    ...
    try {
      ...
      offlineToastShown.current = false // ← reset when back online
    } catch (err) {
      setOfflineError(err as Error)
      if (!offlineToastShown.current) {
        showError('Failed to load services')
        offlineToastShown.current = true // ← only show once
      }
    } finally {
      setLoading(false)
      setRefreshing(false)
  ...
  const handleCreate = async () => {
    if (!name.trim() || !port.trim()) {
      showError('Name and port are required.')
