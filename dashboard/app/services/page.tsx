...
export default function ServicesPage() {
  ...
  const handleCreate = async () => {
    if (!name.trim() || !port.trim()) { setError('Name and port are required.'); return }
    setCreating(true); setError('')
    try {
      await createService(name.trim(), port.trim())
      setModalOpen(false); setName(''); setPort('')
      load(true)
  ...
  const handleDelete = async () => {
    if (!confirmId) return
    setDeleting(true)
    try { await deleteService(confirmId); load(true) }
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-text-primary text-3xl font-semibold tracking-tight mb-1">Services</h1>
          <p className="text-text-muted text-sm">{services.length} service{services.length !== 1 ? 's' : ''} registered</p>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-lg bg-violet/10 border border-violet/20 flex items-center justify-center shrink-0">
                  <Network size={15} strokeWidth={1.5} className="text-violet-lt" />
                </div>
                <div className="min-w-0">
                  <p className="text-text-primary text-sm font-semibold truncate">{svc.name}</p>
                  <p className="text-text-muted text-xs font-mono truncate">{svc.id.slice(0, 8)}…</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-3 py-2.5 px-3 bg-surface rounded-lg border border-border-subtle">
                ...
                <span className="text-text-primary text-xs font-mono font-medium">{svc.port}</span>
              </div>
              ...
              <div>
                ...
                {svc.pods?.length === 0 ? (
                  <p className="text-text-muted text-xs italic">No pods attached</p>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {svc.pods?.map(podId => (
                      <span key={podId} className="text-xs font-mono bg-surface border border-border-subtle text-text-secondary px-2 py-0.5 rounded-md truncate max-w-full">
                        {podId.slice(0, 8)}…
                      </span>
                    ))}
                  </div>
      ...
      <Modal
        open={modalOpen}
        onClose={() => { setModalOpen(false); setError(''); setName(''); setPort('') }}
        title="Create service"
        description="Expose pods through a named service."
      >
        <div className="space-y-4">
          <div>
            <label className="text-text-secondary text-xs font-medium block mb-1.5">Service name</label>
            <input
              value={name}
          ...
          <div>
            <label className="text-text-secondary text-xs font-medium block mb-1.5">Port</label>
            <input
              value={port}
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              onClick={() => { setModalOpen(false); setError(''); setName(''); setPort('') }}
