  ...
  return (
    <aside className="flex flex-col w-56 min-h-screen bg-surface border-r border-border-subtle px-3 py-5">
      ...
      <div className="flex items-center gap-2.5 px-3 mb-8">
        ...
        <span className="text-text-primary font-semibold text-sm tracking-wide">minikube</span>
      </div>
        ...
        {links.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              `}
            >
              <Icon className={isActive ? 'text-violet-lt' : 'text-text-muted'} size={16} />
              {label}
              {isActive && (
      ...
      <div className="mt-auto px-3 pt-4 border-t border-border-subtle">
        ...
        <p className="text-text-muted text-xs mt-0.5 opacity-60">local cluster</p>
      </div>
